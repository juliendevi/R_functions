...
#### Function SRX_to_SRR ----
SRX_to_SRR <- function(list_SRX){
  expID=list()
  runID=list()
  for (i in 1:length(list_SRX)){
    a=entrez_search(db='sra', term = list_SRX[i])
    ...
    expID[i]=list_SRX[i]
    runID[i]=paste(as.character(unlist(c)), collapse=", ")
  }
  df=data.frame(cbind(expID, runID))
  return(df)
}
...
## Input: 
# Vector/list of accession numbers (SRA ID)
## Output: 
